...
def get_permutation(permutations_definition_dict, topology, sequence_ordering, global_type, sidechain_variant, residue_cache=None):
    ...
    before_sidechain_permutation_definition = permutations_definition_dict["backbone"][sequence_ordering]["before_sidechain"]
    after_sidechain_permutation_definition = permutations_definition_dict["backbone"][sequence_ordering]["after_sidechain"]
    ...
    for i, residue in enumerate(residue_list):
        ...
        if residue_cache is not None and residue_cache.get(residue_name_with_terminals) is not None:
            ...
            residue_before_sidechain_permutation = [
                atom_index_by_name[name] for name in before_sidechain_permutation_definition
                if name in atom_index_by_name # Backbone atoms may not be present in all residues
            ]
            residue_after_sidechain_permutation = [
                atom_index_by_name[name] for name in after_sidechain_permutation_definition
                if name in atom_index_by_name # Backbone atoms may not be present in all residues
            ]
    ...
    if global_type == "residue-by-residue":
        ...
        for i in range(topology.n_residues): # Loop through residues once - taking backbone atoms
            ...
            permutation.append(
                torch.tensor([atom_index for atom_index in [*before_sidechain_permutations[i], *after_sidechain_permutations[i]] if atom_index.item() not in hydrogen_indices], dtype=torch.long)
            )
            # store the hydrogen indices separately
            before_sidechain_hydrogen_permutations.append(
                torch.tensor([atom_index for atom_index in before_sidechain_permutations[i] if atom_index.item() in hydrogen_indices], dtype=torch.long)
            )
            after_sidechain_hydrogen_permutations.append(
                torch.tensor([atom_index for atom_index in after_sidechain_permutations[i] if atom_index.item() in hydrogen_indices], dtype=torch.long)
            )
        for i in range(topology.n_residues): # Loop through residues again - taking backbone hydrogens and sidechain atoms
    ...
    with tqdm(total=total, desc="Generating permutations") as pbar: # progress bar for tracking progress
        for sequence_ordering, global_type, sidechain_variant in configs:
            ...
            for sequence, topology in topology_dict.items():
                ...
                permutations_dict[sequence][key + "_flip"] = torch.flip(permutation, dims=[0]) # Also add flipped version of the permutation
