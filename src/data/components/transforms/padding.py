...
class PaddingTransform(torch.nn.Module):
    ...
    def __init__(self, max_num_atoms: int, num_dimensions: int, max_num_residues: int) -> None:
        """
        Args:
    ...
    def pad_data(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pad_tensor = torch.zeros(self.max_num_particles - num_atoms, self.num_dimensions)
    ...
    def forward(self, data: dict[str, Any]) -> dict[str, Any]:
        """
        Args:
            data (Dict[str, Any]): The input data dictionary containing the keys "x", "encoding" 
                                   and possibly "permutations".
        Returns:
            Dict[str, Any]: The updated data dictionary with padded data, encoding, a mask, 
        ...
        atom_mask = self.create_permutation_mask(
            permutations["atom"]["permutations"], padded_seq_len=self.max_num_atoms
        )
        residue_mask = self.create_permutation_mask(
            permutations["residue"]["permutations"], padded_seq_len=self.max_num_residues
        )

        encoding = self.pad_encoding(encoding)

        atom_permutations = self.pad_permutations(
            permutations["atom"]["permutations"], padded_seq_len=self.max_num_atoms
        )
        residue_permutations = self.pad_permutations(
            permutations["residue"]["permutations"], padded_seq_len=self.max_num_residues
        )
        residue_tokenization_map = self.pad_tokenization_map(
            permutations["residue"]["tokenization_map"], padded_seq_len=self.max_num_residues
        )
        ...
        padded_batch = {
            **data,
            ...
            "encoding": encoding,
            "permutations": {
                "atom": {"permutations": atom_permutations, "mask": atom_mask},
                "residue": {
                    "permutations": residue_permutations,
                    "mask": residue_mask,
