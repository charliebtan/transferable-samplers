...
class SinusoidalEmbedding(nn.Module):
    ...
    def forward(self, data):
        ...
        div_term = torch.exp(
            torch.arange(0, self.embed_size, 2, device=data.device)
...
class ConditionalEmbedder(nn.Module):
    def __init__(
        ...
        self.mlp = nn.Sequential(nn.Linear(5 if embed_time else 4 * hidden_dim, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, output_dim))

    def forward(self, atom_type, aa_type, aa_pos, seq_len, t=None, mask=None):
        if mask is None:
            mask = torch.ones_like(atom_type, dtype=torch.bool)
        
        if (t is None) ^ self.embed_time:
