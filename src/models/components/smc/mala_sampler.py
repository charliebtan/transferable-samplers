...
class SMCSamplerMALA(SMCSampler):
    def mcmc_kernel(self, source_energy, target_energy, t, x, logw, dt):
        ...
        energy_grad_x, energy_grad_t = grad_energy_fn(t, x)
        ...
        E = energy_fn(t, x)
        logp = -E_proposal + E
        logp += -0.5 * torch.sum((x - x_proposal + eps * energy_grad_x_proposal) ** 2, dim=-1) / (2 * eps)
        logp -= -0.5 * torch.sum((x_proposal - x + eps * energy_grad_x) ** 2, dim=-1) / (2 * eps)
