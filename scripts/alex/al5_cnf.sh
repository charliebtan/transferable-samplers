...
RUN_NAME="al5_eqcnf_v3"
srun python -u src/train.py experiment=aldp logger=wandb seed=42 data=al5 trainer=ddp \
model/net=egnn_dynamics_ad2_cat tags=[al,cnf,v12,egnn] \
  trainer.check_val_every_n_epoch=200 data.batch_size=1024 \
  trainer.max_epochs=1000 \
  +model.net.pdb_filename='${data.pdb_filename}' \
  model.sampling_config.batch_size=20 +trainer.num_sanity_val_steps=0 \
  model.sampling_config.num_proposal_samples=80 model.net.hidden_nf=256 \
